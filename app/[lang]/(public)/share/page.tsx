...
export default function SharePage() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: t("share.shareTitle", "Institute Management System"),
          text: t("share.shareText", "Check out our school management system!"),
          url: otpUrl,
        })
      } catch (error) {
  ...
  const handleCopy = () => {
    ...
    setCopied(true)
    toast.success(t("share.toastCopied", "URL copied to clipboard"))
  ...
  return (
    <div className="container max-w-md mx-auto py-10 px-4 space-y-6">
      <div className="flex items-center gap-2 mb-4">
        <BackButton />
        <h1 className="text-2xl font-bold">{t("share.pageTitle", "Share App")}</h1>
      </div>
      ...
      <Card className="overflow-hidden border-2 border-primary/10 shadow-xl bg-card">
        <CardHeader className="text-center pb-2 bg-primary/5">
          <CardTitle className="text-xl">{t("share.qrTitle", "App QR Code")}</CardTitle>
          <CardDescription>{t("share.qrSubtitle", "Scan this code to open the app on another device")}</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center pt-8 pb-8 space-y-6">
          ...
          <div className="w-full space-y-3">
            ...
            >
              ...
              {t("share.shareButton", "Share App Link")}
            </Button>
            ...
            >
              ...
              {copied ? t("share.copied", "Copied!") : t("share.copyUrl", "Copy URL")}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <p className="text-center text-sm text-muted-foreground px-4">
        {t("share.footerNote", "Share this app with teachers, parents, and staff to streamline school management.")}
      </p>
    </div>
  )
}
